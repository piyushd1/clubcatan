...
/**
 * Fast string parsing for coordinate keys (e.g., v_0_1_2 or e_0_-1_4)
 * Replaces expensive Regex matching for hot paths.
 * Returns [fullKey, q, r, dir] to maintain compatibility with match() results.
 */
export function parseCoordinateKey(key) {
  if (!key) return null;
  const p1 = 2; // skip "v_" or "e_"
  const p2 = key.indexOf('_', p1);
  if (p2 === -1) return null;
