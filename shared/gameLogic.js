...
/**
 * Fast string parsing for coordinate keys (e.g. v_q_r_dir, e_q_r_dir)
 * Replaces regex matching which is slow in hot paths.
 * Returns an array [key, q, r, dir] matching the regex output format.
 */
export function parseCoordinateKey(key) {
  if (!key || typeof key !== 'string') return null;

  let first = key.indexOf('_');
  if (first === -1) return null;

  let second = key.indexOf('_', first + 1);
  if (second === -1) return null;

  let third = key.indexOf('_', second + 1);
...
/**
 * Fast calculation of total resources.
 * Replaces Object.values(resources).reduce((a, b) => a + b, 0)
 * which creates unnecessary array allocations in hot paths.
 */
