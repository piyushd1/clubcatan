...
/**
 * Fast parser for coordinate keys (e.g. 'v_1_2_3' or 'e_-1_0_5').
 * Replaces expensive regex matching in hot paths.
 * Returns an array-like result [originalKey, q, r, dir] matching regex groups,
 * or null if the string is invalid.
 */
export function parseCoordinateKey(key) {
  if (!key || typeof key !== 'string') return null;
  const u1 = key.indexOf('_');
  if (u1 === -1) return null;
  const u2 = key.indexOf('_', u1 + 1);
  if (u2 === -1) return null;
  const u3 = key.indexOf('_', u2 + 1);
  if (u3 === -1) return null;

  return [
    key,
    key.substring(u1 + 1, u2),
    key.substring(u2 + 1, u3),
    key.substring(u3 + 1)
  ];
}

/**
 * Fast sum of resources avoiding array allocations and callbacks.
 * Schema relies on {brick, lumber, wool, grain, ore}.
 */
