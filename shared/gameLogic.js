...
/**
 * Fast custom parser for coordinate keys (e.g. "v_0_-2_0", "e_-1_2_5").
 * Uses string operations instead of regex to avoid slow match performance in hot paths.
 * Returns array: [originalKey, q, r, dir]
 */
export function parseCoordinateKey(key) {
  if (!key) return null;
  const p1 = key.indexOf('_');
  if (p1 === -1) return null;
  const p2 = key.indexOf('_', p1 + 1);
  if (p2 === -1) return null;
  const p3 = key.indexOf('_', p2 + 1);
  if (p3 === -1) return null;

  return [
    key,
    key.substring(p1 + 1, p2),
    key.substring(p2 + 1, p3),
    key.substring(p3 + 1)
  ];
}

/**
 * Fast resource calculation to avoid Object.values().reduce() array allocation overhead
 */
