...
/**
 * Fast sum of resources in a player's hand.
 * Replaces expensive Object.values(resources).reduce() calls in hot paths.
 */
...
/**
 * Fast string parsing for coordinate keys (e.g., v_0_-1_2 or e_0_-1_2)
 * Replaces expensive regex .match() calls in hot paths.
 * Returns array formatted identically to Regex match result: [fullMatch, q, r, dir]
 */
export function parseCoordinateKey(key) {
  if (!key) return null;
  const parts = key.split('_');
  if (parts.length !== 4) return null;
  return [key, parts[1], parts[2], parts[3]];
}
