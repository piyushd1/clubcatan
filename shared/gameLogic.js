...
// ============================================================================
// KEY GENERATION AND PARSING FUNCTIONS
// ============================================================================

/**
 * Fast custom string parser to replace Regex matching for coordinate keys
 * Parses keys like 'v_1_-2_3' or 'e_0_1_5' into [q, r, dir]
 * Extracted for hot-path optimization
 */
export function parseCoordinateKey(key) {
  if (!key || typeof key !== 'string' || key.length < 5 || key[1] !== '_') return null;

  let q = 0, r = 0, dir = 0;
  let qSign = 1, rSign = 1;
  let i = 2; // skip prefix ('v' or 'e') and first '_'
...
function areVerticesAdjacent(vKey1, vKey2) {
  ...
  const pos1 = getVertexPixelPosition(match1[0], match1[1], match1[2]);
  const pos2 = getVertexPixelPosition(match2[0], match2[1], match2[2]);
...
function giveInitialResources(game, vKey, playerIndex) {
  ...
  const adjacentHexes = getAdjacentHexesToVertex(q, r, dir);
