 ...
 * Expects 'v_q_r_dir' or 'e_q_r_dir' and returns [fullMatch, q, r, dir]
 */
export function parseCoordinateKey(key) {
  if (!key || key.length < 5) return null;
  let idx1 = 2; // skip "v_" or "e_"

  let idx2 = key.indexOf('_', idx1);
  if (idx2 === -1) return null;

  let idx3 = key.indexOf('_', idx2 + 1);
  if (idx3 === -1) return null;

  return [
    key,
    key.slice(idx1, idx2),
    key.slice(idx2 + 1, idx3),
    key.slice(idx3 + 1)
  ];
}
