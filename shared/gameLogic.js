...
/**
 * Fast custom string parser to replace Regex for coordinate keys.
 * Expects keys like 'v_1_-2_3' or 'e_0_1_2'.
 * Returns an array: [fullMatch, q, r, dir]
 */
export function parseCoordinateKey(key) {
  if (!key) return null;
  const firstUnderscore = key.indexOf('_');
  if (firstUnderscore !== 1) return null;
  const secondUnderscore = key.indexOf('_', 2);
  if (secondUnderscore === -1) return null;
  const thirdUnderscore = key.indexOf('_', secondUnderscore + 1);
  if (thirdUnderscore === -1) return null;

  return [
    key,
    key.slice(2, secondUnderscore),
    key.slice(secondUnderscore + 1, thirdUnderscore),
    key.slice(thirdUnderscore + 1)
  ];
}
