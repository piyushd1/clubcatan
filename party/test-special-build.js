// Test suite for canSpecialBuild function in gameLogic.js
...
function testCanSpecialBuild() {
  ...
  log('\n--- Scenario: Special Building Phase is active ---', 'magenta');
  {
    ...
    game.specialBuildIndex = 1; // Bob's turn (index 1)

    assert(GameLogic.canSpecialBuild(game, 'p1') === false, 'Returns false for Alice (index 0) when it is Bob\'s turn');
    assert(GameLogic.canSpecialBuild(game, 'p2') === true, 'Returns true for Bob (index 1) when it is his turn');
    assert(GameLogic.canSpecialBuild(game, 'p3') === false, 'Returns false for Charlie (index 2) when it is Bob\'s turn');
  }
