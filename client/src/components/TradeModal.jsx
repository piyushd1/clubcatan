    ...
    return (
      <div className="modal-overlay" onClick={onClose}>
        <div className="trade-modal" onClick={e => e.stopPropagation()}>
          <button aria-label="Close trade" className="close-btn" onClick={onClose}>×</button>
                      ...
                      {RESOURCES.map(r => (
                        <div key={r} className="resource-selector">
                          ...
                          <div className="selector-controls">
                            <button aria-label={`Decrease offer ${r}`} onClick={() => updateOffer(r, -1)} disabled={offer[r] === 0}>−</button>
                            <span className="amount">{offer[r]}</span>
                            <button aria-label={`Increase offer ${r}`} onClick={() => updateOffer(r, 1)} disabled={offer[r] >= myPlayer.resources[r]}>+</button>
                          </div>
                    ...
                    <div className="resource-selectors">
                      {RESOURCES.map(r => (
                        <div key={r} className="resource-selector">
                          <span className="icon">{RESOURCE_ICONS[r]}</span>
                          <div className="selector-controls">
                            <button aria-label={`Decrease request ${r}`} onClick={() => updateRequest(r, -1)} disabled={request[r] === 0}>−</button>
                            <span className="amount">{request[r]}</span>
                            <button aria-label={`Increase request ${r}`} onClick={() => updateRequest(r, 1)}>+</button>
                          </div>
                        </div>
                      ))}
                    </div>
