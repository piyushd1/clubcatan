    ...
    return (
      <div className="modal-overlay" onClick={onClose}>
        <div className="trade-modal" onClick={e => e.stopPropagation()}>
          <button className="close-btn" aria-label="Close trade modal" onClick={onClose}>×</button>
                      ...
                      {RESOURCES.map(r => (
                        <div key={r} className="resource-selector">
                          ...
                          <div className="selector-controls">
                            <button onClick={() => updateOffer(r, -1)} disabled={offer[r] === 0} aria-label={`Decrease ${r} offer`}>−</button>
                            <span className="amount">{offer[r]}</span>
                            <button onClick={() => updateOffer(r, 1)} disabled={offer[r] >= myPlayer.resources[r]} aria-label={`Increase ${r} offer`}>+</button>
                          </div>
                    ...
                    <div className="resource-selectors">
                      {RESOURCES.map(r => (
                        <div key={r} className="resource-selector">
                          <span className="icon">{RESOURCE_ICONS[r]}</span>
                          <div className="selector-controls">
                            <button onClick={() => updateRequest(r, -1)} disabled={request[r] === 0} aria-label={`Decrease ${r} request`}>−</button>
                            <span className="amount">{request[r]}</span>
                            <button onClick={() => updateRequest(r, 1)} aria-label={`Increase ${r} request`}>+</button>
                          </div>
                        </div>
                      ))}
                    </div>
